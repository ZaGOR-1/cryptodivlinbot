...
| `PRIVACY_POLICY_URL` | _GitHub `docs/PRIVACY_POLICY.md`_ | Ссылка, которую отдаёт `/privacy`. |
| `TERMS_OF_SERVICE_URL` | _GitHub `docs/TERMS_OF_SERVICE.md`_ | Ссылка, которую отдаёт `/terms`. |
| `SENTRY_DSN` | _(пусто)_ | DSN Sentry / GlitchTip. Пусто = мониторинг отключён. Требует `pip install '.[monitoring]'`. |
| `SENTRY_ENVIRONMENT` | `production` | Тег среды в Sentry. |
| `SENTRY_TRACES_SAMPLE_RATE` | `0.0` | Доля трассировки производительности (0.0 – 1.0). |
...
| `/privacy` | Кратко: какие данные хранит бот, плюс ссылка на полную [Политику конфиденциальности](./PRIVACY_POLICY_RU.md). |
| `/terms` | Кратко: условия использования, плюс ссылка на полный [текст](./TERMS_OF_SERVICE_RU.md). |
| `/forgetme` | GDPR-удаление данных. Первый вызов — подтверждение; `/forgetme yes` безвозвратно удаляет всё, что бот хранит о чате. |

---

## 6. Кнопки

Бот предлагает **два** варианта кнопок одновременно — выбирайте, что удобнее.

### 6.1. Закреплённая клавиатура внизу чата (ReplyKeyboard)

После `/start` под полем ввода появляется постоянная клавиатура с
кнопками, которая **остаётся на месте между сообщениями**
(`is_persistent=True`, `resize_keyboard=True`). Тап по кнопке — бот
выполняет соответствующее действие, как если бы вы ввели команду:

- **🔔 Подписаться** — то же, что `/subscribe`.
- **🔕 Отписаться** — то же, что `/unsubscribe`.
- **📊 Статус** — `/status`.
- **📰 Дайджест сейчас** — `/digest`.
- **🪙 Монеты** — `/coins`.
- **🌐 Язык** — `/language` (открывает inline-меню выбора языка).
- **❓ Справка** — `/help`.

Подписи на кнопках автоматически переводятся — после `/setlang uk` или
выбора языка inline-кнопкой клавиатура внизу обновляется с украинскими
лейблами. Это заложено: `setlang` и callback `lang:<code>` повторно
отправляют клавиатуру с новым набором подписей.

### 6.2. Inline-меню в сообщении

Команда `/menu` открывает компактное inline-меню с теми же действиями
(и дополнительно кнопками **⬅️ Назад** и **✖️ Закрыть**). Оно удобнее
для сценариев, в которых нужно делать серию действий в рамках одного
сообщения (`Назад` возвращает в главное меню без отправки нового).
Кнопка **🌐 Язык** открывает `language_menu` с флагами.
...
- `restart: unless-stopped` — автоматический рестарт при падении
  процесса, OOM-кила, рестарте демона Docker и ребуте хоста. В
  отличие от `always`, уважает явный `docker stop` (вручную
  остановленный контейнер не оживает сам).
- `init: true` — `tini` в роли PID 1 корректно прокидывает SIGTERM
  в Python, чтобы PTB мог чисто завершить работу (закрыть WAL,
  остановить JobQueue) вместо SIGKILL.
- `stop_grace_period: 30s` — даём PTB время на завершение текущего
  цикла опроса перед SIGKILL.
...
> Без Docker? Смотрите раздел
> [«Run as a systemd service»](../README.md#run-as-a-systemd-service)
> в `README.md` — готовый
> [`deploy/cryptodivlinbot.service`](../deploy/cryptodivlinbot.service)
> с `Restart=on-failure`, ограничением crash-loop'ов и усиленным
> sandbox'ом (работает на VPS / Raspberry Pi).
